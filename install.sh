#!/bin/bash
# Bitsness AI Marketing Suite — Claude Code Skills Installer
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║   Bitsness AI Marketing Suite — Claude Code  ║${NC}"
...
if [ -n "$BASH_SOURCE" ] && [ "$BASH_SOURCE" != "bash" ] && [ -f "$BASH_SOURCE" ]; then
    ...
    TEMP_DIR=$(mktemp -d)
    git clone --depth 1 https://github.com/GodOfIT/bitsness-ai-marketing-claude.git "$TEMP_DIR/bitsness-ai-marketing-claude" 2>/dev/null
    if [ $? -ne 0 ]; then
        echo -e "${RED}Failed to clone repository.${NC}"
        exit 1
    fi
    SCRIPT_DIR="$TEMP_DIR/bitsness-ai-marketing-claude"
fi
...
# Install logo
if [ -f "$SCRIPT_DIR/resources/logo.jpg" ]; then
    cp "$SCRIPT_DIR/resources/logo.jpg" "$SKILLS_DIR/market/logo.jpg"
    echo -e "  ${GREEN}✓${NC} logo.jpg"
fi
